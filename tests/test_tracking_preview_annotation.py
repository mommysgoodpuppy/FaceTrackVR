...
def test_tracking_preview_annotates_pupil_and_diagnostics():
    ...
    changed_dilation = EyeInfo(
        ...
        eyebrow=0.4,
        squeeze=0.2,
        auxiliary_diagnostics=eye_info.auxiliary_diagnostics,
    )
    widget._annotate_tracking_preview(image, changed_dilation)

    # Reusing the same bounded sample_count must not freeze output history.
    assert list(widget._dilation_history) == [0.7, 0.3]
